...
/**
 * Servlet implementation class SearchDeleteReviewServlet
 */
public class SearchDeleteReviewServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SearchDeleteReviewServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		HashMap<String, Object> map = new HashMap<String, Object>();
			...
			reviews = new ReviewService().searchDeleteReviewUserNo(searchWord);
			map.put("review", reviews);
			map.put("searchWord", searchWord);
		
			}else if(option.equals("rwNo")){
			reviews = new ReviewService().searchDeleteReviewRwNo(searchWord);
			map.put("review", reviews);
			map.put("searchWord", searchWord);
				...
				reviews = new ReviewService().searchDeleteCategoryType(categoryType);
				map.put("review", reviews);
				map.put("categoryType", categoryType);
			}else if(option.equals("rwCount")){
				reviews = new ReviewService().searchDeleteRwCount(searchWord);
				map.put("review", reviews);
				map.put("searchWord", searchWord);
				...
				reviews = new ReviewService().searchDeleteRwcontentType(rwcontentType);
				map.put("review", reviews);
				map.put("rwcontentType", rwcontentType);
			
			}else if(option.equals("cancleDate")){
				
				reviews = new ReviewService().searchDeleteCancleDate(searchWord);
				map.put("review", reviews);
				map.put("searchWord", searchWord);
				...
				listCount = new ReviewService().getDeletelistCount();
				...
				reviews = new ReviewService().DeleteSearchAll(currentPage, limit);
